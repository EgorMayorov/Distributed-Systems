...
static void err_handler (MPI_Comm *pcomm, int *perr, ...) {
    ...
    printf("Process with rank %d know about an error %s.\n", rank, errstr);
...
void parallel_odd_even_sort (int n, int *a) {
    ...
    for (int i = 1; i <= tasks - 1; i++) {
        // если случилась ошибка в каком-то процессе, остальные процессы заново начинают итерацию
        checkpoint:
        MPI_Barrier(main_comm);
...
// При запуске программы нужно сразу запустить на один MPI-процесс больше, на случай, если один из процессов откажет,
// дополнительный процесс начнет работу в случае сбоя.
